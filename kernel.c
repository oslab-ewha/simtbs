...
void insert_kernel(unsigned start_ts, unsigned n_tb, unsigned tb_rsc_req, unsigned tb_mem_rsc_req, unsigned tb_duration)
{
	kernel_t *kernel;
...
void check_new_arrived_kernel(void)
{
...
preprocess_tb(void)
{
    ...
    tb_t_list *tb_bucket_list = (tb_t_list *)malloc(sizeof(tb_t_list) * (sm_rsc_max + 1));
...
void save_conf_kernel_infos(FILE *fp)
{
	struct list_head *lp;
