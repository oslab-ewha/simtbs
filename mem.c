...
void setup_mem(unsigned conf_mem_rsc_max)
{
...
float get_overhead_mem_SA(unsigned rsc)
{
	return get_mem_overhead(rsc);
}

float get_overhead_mem(unsigned rsc)
{
	return get_mem_overhead(mem_rsc_used) * rsc / mem_rsc_used;
}

void insert_overhead_mem(unsigned to_rsc, float tb_overhead)
{
	overhead_t *oh;
...
void check_overhead_sanity_mem(void)
{
	overhead_t *oh_last;
...
void assign_mem(unsigned mem_rsc_req)
{
...
void revoke_mem(unsigned mem_rsc_req)
{
...
void save_conf_mem_overheads(FILE *fp)
{
	struct list_head *lp;
